...
describe("profile actions calling API should dispatch correctly", () => {
  ...
  describe("delete profile video dispatch correctly", () => {
    ...
    it("delete video succeeds", () => {
      ...
      const store = mockStore({});
      return store
        .dispatch(actions.deleteVideo(1, closeModal, stopLoading))
        .then(() => {
          expect(store.getActions()[0]).toEqual(expectedActionOne);
    ...
    it("delete video fails", () => {
      const store = mockStore({});
      return store
        .dispatch(actions.deleteVideo(0, closeModal, stopLoading))
        .then(() => {
    ...
    it("No video id provided", () => {
      const store = mockStore({});
      return store
        .dispatch(actions.deleteVideo("", closeModal, stopLoading))
        .then(() => {
          expect(store.getActions()[0].type).toEqual(componentTypes.SET_ALERT);
          expect(store.getActions()[0].payload.msg).toEqual(
  ...
  describe("change profile picture dispatches correctly", () => {
    ...
    it("profile picture changed failure dispatches error actions", () => {
      ...
      return store
        .dispatch(actions.changePicture(file, closeModal))
        .then(() => {
          expect(store.getActions()[0]).toEqual(expectedActionOne);
          expect(store.getActions()[1].type).toEqual(componentTypes.SET_ALERT);
    ...
    it("profile picture size limited exceeded dispatches error actions", () => {
      ...
      window.localStorage.setItem("user", 1);
      return store
        .dispatch(actions.changePicture(bigFile, closeModal))
        .then(() => {
          expect(store.getActions()[0]).toEqual(expectedActionOne);
          expect(store.getActions()[1].type).toEqual(componentTypes.SET_ALERT);
          expect(store.getActions()[2]).toEqual(expectedActionThree);
    ...
    it("empty file selected dispatches error actions", () => {
      ...
      window.localStorage.setItem("user", 1);
      return store
        .dispatch(actions.changePicture(undefined, closeModal))
        .then(() => {
          expect(store.getActions()[0]).toEqual(expectedActionOne);
          expect(store.getActions()[1].type).toEqual(componentTypes.SET_ALERT);
          expect(store.getActions()[1].payload.msg).toEqual(
    ...
    const expectedActionTwo = {
      ...
      payload: {
        basic: {
          id: 27,
          profile_pic:
            "https://thinkslice-project.s3.amazonaws.com/user-images/barrett_l7rngxa.jpg",
          username: "gyro",
      ...
      return store
        ...
        .then(() => {
          ...
          expect(store.getActions()[2].type).toEqual(
            types.STOP_REVIEW_POST_LOADING
          );
        });
    });
        ...
        },
        {
          ...
          payload: {
            basic: {
              id: 3,
              profile_pic:
                "https://thinkslice-project.s3.amazonaws.com/user-images/",
              username: "Applesauce",
              user_bio: "Hi, welcome to my profile!",
              is_tutor: true,
              is_student: false,
            ...
            detailed: {
              ...
              subjects: ["Math", "Cooking", "Biology", "Business", "Computing"],
              total_tutor_reviews: 0,
            ...
            reviewsGiven: [
              {
                ...
                creator_details: {
                  profile_pic:
                    "https://thinkslice-project.s3.amazonaws.com/user-images/shrug_gumball_by_bornreprehensible-d7mr2jr.png?AWSAccessKeyId=AKIA3EDWA4JQ57MY2PM5&Signature=QxXXShDOIs4xmiK8HNUIXCpshVk%3D&Expires=1623560876",
                  username: "Pearsauce",
                  user: 1,
                },
                star_rating: 5.0,
                ...
                review_essay:
                  "You won't believe what this guy would do for $5!",
            ...
            reviewsReceived: [
              {
                ...
                creator_details: {
                  profile_pic:
                    "https://thinkslice-project.s3.amazonaws.com/user-images/download.jpg?AWSAccessKeyId=AKIA3EDWA4JQ57MY2PM5&Signature=GZHI1BfgS7c1RZiErYdiIQ3ouZ4%3D&Expires=1623560877",
                  username: "Applesauce",
                  user: 3,
                },
                star_rating: 4.0,
